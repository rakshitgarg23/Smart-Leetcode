...
(
    ...
    '${JSON.stringify(twoSumStarter).replace(/'/g, "''")}'::jsonb
),
(
    'Valid Parentheses',
    3,
    ...
    '${JSON.stringify(validParenthesesStarter).replace(/'/g, "''")}'::jsonb
),
(
    ...
    '${JSON.stringify(containerWaterStarter).replace(/'/g, "''")}'::jsonb
),
(
    '3Sum',
    1,
    ...
    '${JSON.stringify(threeSumStarter).replace(/'/g, "''")}'::jsonb
),
(
    'Reverse Linked List',
    4,
    ...
    '${JSON.stringify(reverseListStarter).replace(/'/g, "''")}'::jsonb
),
(
    'Linked List Cycle',
    4,
    'Easy',
    'Given head, the head of a linked list, determine if the linked list has a cycle in it. (pos represents the index of the node that tail connects to).',
    '[
    ...
    '${JSON.stringify(hasCycleStarter).replace(/'/g, "''")}'::jsonb
),
(
    'Daily Temperatures',
    5,
    ...
    '${JSON.stringify(dailyTemperaturesStarter).replace(/'/g, "''")}'::jsonb
),
(
    ...
    '${JSON.stringify(maxSlidingWindowStarter).replace(/'/g, "''")}'::jsonb
),
(
    'Invert Binary Tree',
    7,
    ...
    '[
        ...
        {"input": "root = [2,1,3]", "expected_output": "[2,3,1]"},
        {"input": "root = []", "expected_output": "[]"}
    ]'::jsonb,
    '${JSON.stringify(invertTreeStarter).replace(/'/g, "''")}'::jsonb
),
(
    ...
    '${JSON.stringify(levelOrderStarter).replace(/'/g, "''")}'::jsonb
),
(
    ...
    '${JSON.stringify(isValidBSTStarter).replace(/'/g, "''")}'::jsonb
);
`;

fs.writeFileSync('database/seed.sql', newSeed);
